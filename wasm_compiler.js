...
/*# sourceMappingURL=${css.map.toUrl()} */` : ""}</style>\`;`}
  ...
  function InlineComponent$1(node2, renderer, options) {
    ...
    if (children.length) {
      ...
      slot_scopes.forEach(({ input, output: output2, statements }, name) => {
        slot_fns.push(p`${name}: (${input}) => { ${statements}; return ${output2}; }`);
      });
    }
    const slots = x`{
		${slot_fns}
...
${frame}`
  ...
  function o(i) {
    ...
    for (; ; ) {
      ...
      if (e += t, e === r2.length) {
        const n2 = new Uint8Array(r2.length * 2);
        n2.set(r2), r2 = n2;
      }
    }
  }
  ...
  // wasm_compiler.ts
  var inputBytes = o(r.Stdin);
  var inputString = new TextDecoder().decode(inputBytes);
  var output = compile2(inputString);
  var outputBytes = new TextEncoder().encode(output);
  l(r.Stdout, outputBytes);
})();
